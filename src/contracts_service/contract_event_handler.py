...
def create_contract(event: dict) -> None:
    ...
    logger.info(f"Creating contract: {contract} From event: {event}")
...
def update_contract(contract: dict) -> None:
    ...
    try:
        ...
        code = e.response["Error"]["Code"]
        if code == 'ConditionalCheckFailedException':
            logger.error(f"Unable to update contract Id {contract['property_id']}. Status is not in status DRAFT")
        elif code == 'ResourceNotFoundException':
            logger.error(f"Unable to update contract Id {contract['property_id']}. Not Found")
        else:
            raise e